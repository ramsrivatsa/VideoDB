...
public class FeatureExtractionOp extends OpenCVOp<CVParticle> implements ISingleInputOperation<CVParticle> {
	...
	public List<CVParticle> execute(CVParticle particle) throws Exception {
		...
		logger.info("Feature Extraction start: " + " Sequence Nr - " + particle.getSequenceNr() + " Stream Id - " + particle.getStreamId() + " System Time - " + System.currentTimeMillis());
		...
		try{
			...
			logger.info("Feature Extraction end: " + " Sequence Nr - " + frame.getSequenceNr() + " Stream Id - " + frame.getStreamId() + " System Time - " + System.currentTimeMillis());
		}catch(Exception e){
