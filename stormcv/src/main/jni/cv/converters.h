#ifndef CV_CONVERTERS_H
#define CV_CONVERTERS_H

#include "features2d_converters.h"
...
void Mat_to_vector_vector_float(cv::Mat &mat, std::vector<std::vector<float>> &vv_fl);
void vector_vector_float_to_Mat(std::vector<std::vector<float>> &vv_fl, cv::Mat &mat);
